...
def init_db():
    ...
    cursor.execute("INSERT OR REPLACE INTO configuracoes (chave, valor) VALUES ('precos', ?)", (json.dumps(default_precos),))
...
def get_metricas_financeiras():
    ...
    for r in rows:
        st = r["status"] if r["status"] else "Em Negociação"
...
CURRENT_DATA = {
    ...
    "data_entrega_prevista": (date.today() + timedelta(days=30)).strftime("%Y-%m-%d"),
    "entrada_valor": 0.0,
    "num_parcelas": 1,
...
# MOTOR DE CÁLCULO ESCALONÁVEL PARA PLANTAS COMPACTAS OU GRANDES (> 160m² até 1000m²+)
def calcular_engenharia_avancada(
    ...
    # Coeficiente de modulação por cômodo considerando a metragem total
    area_por_comodo = area_val / qtd_ambientes

    for amb in ambientes_selecionados:
        if "Cozinha" in amb or "Gourmet" in amb:
            # Em plantas >160m², a cozinha e área gourmet ganham ilhas, torres quentes duplas e aéreos estendidos
            ...
            if area_val >= 160:
                # Ilha central / torre quente estendida para plantas amplas
                items.append({
            ...
            items.append({"nome": f"Fita de Borda PVC ({fabricante_mdf})", "tipo": "Fita de Borda", "ambiente": amb, "largura": 0, "altura": 0, "dimensoes": "-", "qtd": int(m_lin * 20), "valor": (m_lin * 20) * precos["fita_borda_m"]})
            ...
            items.append({"nome": f"Divisórias, Prateleiras e Maleiros ({amb})", "tipo": "Chapa MDF / Painel", "ambiente": amb, "largura": 900, "altura": 550, "dimensoes": "900 x 550 x 18 mm", "qtd": num_portas * 2, "valor": (num_portas * 2) * (0.9 * 0.55 * custo_m2_mdf_base)})
        ...
        else: # Lavanderia / Varanda / Adega / Outros
...
def render_pagina_captacao(sucesso=False, orc_id=None, estimativa=0.0, zap_url=""):
    ...
    <div class="bg-amber-950/40 border border-amber-500/50 p-6 sm:p-8 rounded-3xl text-center space-y-4 shadow-2xl">
        ...
        <h2 class="text-xl font-bold text-white">Planta & Projeto MVI Calculados com Sucesso!</h2>
        ...
        <p class="text-xs text-slate-300">
            Você está sendo redirecionado para o WhatsApp da <b>{empresa['nome_empresa']}</b> com seu briefing de alto padrão...
        </p>
    ...
    <form action="/enviar-solicitacao-lead" method="post" enctype="multipart/form-data" class="space-y-4 bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-2xl">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                ...
                <input type="number" step="any" min="5.0" max="5000.0" name="area_m2_total" value="180.0" placeholder="Ex: 180.5" required class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-amber-500">
                <span class="text-[10px] text-slate-500 block mt-1">Ex: 180 m² (Apartamento Alto Padrão), 250 m² (Casa/Cobertura) ou 40 m².</span>
            </div>
            <div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                ...
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    <input type="checkbox" name="ambientes_check" value="Área Gourmet / Varanda" checked class="rounded text-amber-500">
                ...
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    ...
                    <span>🛏️ Suíte Master / Closet</span>
                </label>
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    <input type="checkbox" name="ambientes_check" value="Suíte 2 / Dormitório" checked class="rounded text-amber-500">
                    <span>🧸 Suíte 2 / Hóspedes</span>
                </label>
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    <input type="checkbox" name="ambientes_check" value="Home Cinema / Living" checked class="rounded text-amber-500">
                    <span>📺 Home Cinema / Sala</span>
                </label>
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    <input type="checkbox" name="ambientes_check" value="Banheiros / Lavabos" checked class="rounded text-amber-500">
                    <span>🚿 Banheiros / Lavabo</span>
                </label>
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                ...
                <label class="flex items-center space-x-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800 cursor-pointer hover:border-amber-500">
                    <input type="checkbox" name="ambientes_check" value="Home Office / Adega" class="rounded text-amber-500">
                    <span>🍷 Office / Adega</span>
                </label>
            </div>
        </div>

        <div class="bg-slate-950 p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-4">
            <h3 class="text-xs font-bold text-amber-400 uppercase tracking-wide">🪵 Catálogo de Fabricantes & Acabamentos</h3>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs pt-1">
                <div>
                    ...
                    <select name="espessura_caixa" class="w-full px-3 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white">
                        <option value="MDF 18mm">MDF 18mm (Reforçado / Alto Padrão)</option>
                ...
                <div>
                    ...
                    <select name="espessura_tamponamento" class="w-full px-3 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white">
                        <option value="Tamponamento 25mm">Tamponamento 25mm (Robusto)</option>
                        <option value="Tamponamento 36mm Engrossado">Tamponamento 36mm Engrossado (Luxo)</option>
        ...
        <div>
            <label class="block text-xs font-semibold text-slate-300 uppercase mb-1">Detalhes Adicionais do Imóvel</label>
            <textarea name="descricao" rows="2" placeholder="Ex: Pé direito duplo no living, adega climatizada embutida, closet master com ilha de joias..." class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-amber-500"></textarea>
        </div>
...
async def enviar_solicitacao_lead(
    ...
    obs_completa = f"Lead Imóvel {area_m2_total}m² ({cidade}) | MDF: {fabricante_mdf} ({cor_mdf}) | Ferragens: {marca_ferragens} | Portas: {modelo_portas} | Caixa: {espessura_caixa} | Tamp: {espessura_tamponamento}"
    if descricao:
        obs_completa += f" | Detalhes: {descricao}"

    prazo_calc = "35 a 45 dias úteis" if area_m2_total >= 160 else "25 dias úteis"
...
def gerar_os(id: int = None):
    ...
    if id:
        ...
        c_nome = CURRENT_DATA['cliente_nome'] or "Cliente"
        c_tel = CURRENT_DATA['cliente_telefone']
        c_amb = CURRENT_DATA['cliente_ambiente'] or "Geral"
    ...
    meta_data = [
        ...
        ["Ambientes:", ", ".join(ambientes_list) if ambientes_list else "-", "Prazo:", c_prazo]
    ]
...
def gerar_pdf(id: int = None):
    ...
    cliente_data = [
        ...
        ["Ambientes Inclusos:", ", ".join(ambientes_list) if ambientes_list else "-", "Status:", c_status]
    ]
    ...
    dre_data = [
        ["Ambientes do Projeto", f"{', '.join(ambientes_list) if ambientes_list else '-'}"],
...
def gerar_vistoria(id: int = None):
    ...
    meta_data = [
        ...
        ["WhatsApp/Tel:", c_tel, "Ambientes Montados:", ", ".join(ambientes_list) if ambientes_list else "-"]
    ]
    meta_table = Table(meta_data, colWidths=[110, 160, 120, 150])
    meta_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#f8fafc')),
...
def gerar_contrato(id: int = None):
    ...
    p_obj = f"O presente contrato tem por objeto a fabricação, acabamento e instalação de móveis sob medida pela <b>MVI Móveis Planejados</b> destinados aos seguintes ambientes: <b>{', '.join(ambientes_list) if ambientes_list else '-'}</b>, em conformidade com o projeto executivo e relação de insumos aprovados."
