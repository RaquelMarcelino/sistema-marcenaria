...
DB_PATH = "mvi_production_master_v1.db"
...
def get_empresa_dados(empresa_id=1):
    ...
    return {"id": 1, "slug": "mvi", "nome_empresa": "MVI Móveis", "telefone": "", "chave_mestra": "MVI2026"}
...
def calcular_engenharia(ambientes: list, area_m2: float, exp_caixa: str, exp_tamp: str, fab_mdf: str, cor_mdf: str, mod_portas: str, marca_ferr: str):
    ...
    custo_base = mdf_preco * fator_caixa * fator_tamp * fator_mdf * fator_portas
    area = max(area_m2, 5.0)
    qtd_amb = max(len(ambientes), 1)
    area_com = area / qtd_amb

    items, desc_promob = [], []
    for amb in ambientes:
        m_lin = max(area_com * 0.32, 3.2 if area >= 160 else 2.2)
        n = max(int(math.ceil(m_lin / 0.8)), 2)
        items.append({"nome": f"Caixaria ({exp_caixa}) - {amb}", "valor": n * 1.25 * custo_base})
        items.append({"nome": f"Portas ({fab_mdf})", "valor": n * 2 * 0.58 * custo_base})
        items.append({"nome": f"Dobradiças ({marca_ferr})", "valor": n * 4 * dob_preco * dob_mult})
        items.append({"nome": f"Corrediças ({marca_ferr})", "valor": 4 * corr_preco * corr_mult})
        desc_promob.append(f"{amb}: {n} módulos caixaria {exp_caixa}, portas {fab_mdf} ({cor_mdf}), ferragens {marca_ferr}.")

    t_mat = sum(i["valor"] for i in items)
    t_mo = max(int(math.ceil(qtd_amb * 3.0)), 4) * 180.0
    t_frete = max(qtd_amb * 400.0, 800.0)
    pv = (t_mat + t_mo + t_frete) * 2.2
    lucro = pv - (t_mat + t_mo + t_frete + (pv * 0.10))

    return {"items": items, "total_mat": t_mat, "preco_venda": pv, "lucro": lucro, "desc_promob": "\n".join(desc_promob)}
...
def processar_arquivo_promob(conteudo_texto: str, nome_arquivo: str):
    ...
    if nome_arquivo.lower().endswith(".xml"):
        try:
            ...
            for el in root.iter():
                if el.tag.lower() in ["item", "peca"]:
                    larg = float(el.attrib.get("WIDTH") or 0)
                    alt = float(el.attrib.get("HEIGHT") or 0)
                    qtd = int(float(el.attrib.get("QUANTITY") or 1))
                    if larg > 0 and alt > 0:
                        items.append({"valor": (larg/1000)*(alt/1000) * mdf_preco * 1.3 * qtd})
        except: pass

    if not items:
        items.append({"valor": 4 * 1.25 * mdf_preco})
        items.append({"valor": 16 * dob_preco * 2.8})

    t_mat = sum(i["valor"] for i in items)
    pv = (t_mat + 500 + 400) * 2.2
    lucro = pv - (t_mat + 500 + 400 + (pv * 0.10))
    return {"items": items, "preco_venda": pv, "lucro": lucro}
...
def render_dashboard_view():
    ...
    for u in equipe:
        ...
        st_acesso = f"<div class='flex gap-2'><span class='text-emerald-400 font-bold'>✓ Ativo</span><form action='/redefinir-senha-funcionario' method='post' class='inline'><input type='hidden' name='email_funcionario' value='{u['email']}'><button type='submit' class='px-2 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded text-[10px]'>Reset Senha</button></form></div>" if u["primeiro_acesso_concluido"] else f"<a href='/primeiro-acesso/{u['token_primeiro_acesso']}' target='_blank' class='px-2 py-1 bg-amber-500/20 text-amber-300 rounded text-[10px] underline'>Link Convite</a>"
        ...
        <div id="aba-equipe" class="tab-content space-y-6">
            <div class="grid lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow">
                    <h2 class="text-base font-bold text-white mb-4">Cadastrar Usuário</h2>
                    <form action="/criar-usuario" method="post" class="grid sm:grid-cols-2 gap-3 text-xs">
                        <input type="text" name="nome" required placeholder="Nome" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                        ...
                        <input type="text" name="telefone" placeholder="WhatsApp" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                        ...
                        <button type="submit" class="col-span-full py-3 bg-amber-500 text-slate-950 font-bold rounded-xl">Gerar Convite</button>
                    </form>
                </div>
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow"><h3 class="text-xs font-bold text-slate-300 uppercase mb-3">Equipe</h3><ul class="divide-y divide-slate-800">{eq_html}</ul></div>
            </div>
        </div>
        <div id="aba-config" class="tab-content space-y-6">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow">
                <h2 class="text-base font-bold text-white mb-4">Configuração e Chave Mestra</h2>
                <form action="/salvar-empresa" method="post" class="grid sm:grid-cols-4 gap-4 text-xs">
                    <input type="text" name="nome_empresa" value="{empresa['nome_empresa']}" required class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                    <input type="text" name="cnpj" value="{empresa.get('cnpj','')}" required class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                    <input type="text" name="telefone" value="{empresa['telefone']}" required class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                    <input type="text" name="chave_mestra" value="{empresa['chave_mestra']}" required class="px-3 py-2 bg-slate-950 border border-amber-500/50 rounded-xl text-amber-300 font-bold">
                    <button type="submit" class="col-span-full py-3 bg-amber-500 text-slate-950 font-bold rounded-xl">Salvar</button>
                </form>
            </div>
        </div>
    """ if is_admin else ""
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans">
    ...
    <nav class="bg-slate-900/80 border-b border-slate-800 px-6 py-3 sticky top-0 z-50 backdrop-blur-md">
        <div class="max-w-7xl mx-auto flex items-center gap-2 overflow-x-auto">
            ...
            <button onclick="mudarAba('aba-cadastro')" id="btn-aba-cadastro" class="tab-btn px-4 py-2 rounded-xl text-xs font-bold border border-slate-700 text-slate-300">📋 Ficha & Contrato</button>
    ...
    <main class="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div id="aba-promob" class="tab-content space-y-6">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow space-y-4">
                <h2 class="text-base font-bold text-white">🚀 Integrador Promob Universal</h2>
                <form action="/importar-promob" method="post" enctype="multipart/form-data" class="grid sm:grid-cols-3 gap-4 text-xs">
                    ...
                    <div class="sm:col-span-3 p-4 bg-slate-950 border border-slate-800 rounded-2xl">
                        <label class="block font-bold text-amber-400 mb-2">📁 Arquivo Promob (.xml, .csv, .txt, .cut)</label>
                        <input type="file" name="arquivo_promob" required class="w-full text-slate-400 file:bg-amber-500 file:border-0 file:rounded-xl file:px-4 file:py-2 file:font-bold">
                    </div>
                    <button type="submit" class="sm:col-span-3 py-3.5 bg-amber-500 font-bold rounded-xl text-slate-950 text-sm">⚡ Processar Promob & Gerar Orçamento</button>
                </form>
            </div>
        </div>
        ...
        <div id="aba-cadastro" class="tab-content space-y-6">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow space-y-4">
                <h2 class="text-base font-bold text-white">📑 Ficha Cadastral e Geração de Contrato</h2>
                <form action="/salvar-dados-completos-cliente" method="post" class="space-y-4 text-xs">
                    <select name="orcamento_id" class="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white mb-4">
                        {"".join([f"<option value='{h['id']}'>#{h['id']} - {h['cliente_nome']}</option>" for h in leads])}
                    </select>
                    <div class="grid sm:grid-cols-4 gap-3">
                        ...
                        <input type="text" name="cliente_rg" required placeholder="RG" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                        <input type="text" name="cliente_rg_emissor" placeholder="Órgão Emissor" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                        ...
                        <input type="text" name="cliente_telefone" required placeholder="WhatsApp" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                    </div>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div class="bg-slate-950 p-4 border border-slate-800 rounded-xl">
                            <label class="font-bold text-white block mb-2">Endereço Postal</label>
                            <input type="text" name="cliente_cep_postal" placeholder="CEP" class="w-full mb-2 px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white">
                            <textarea name="cliente_endereco_postal" rows="2" placeholder="Logradouro..." class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"></textarea>
                        </div>
                        <div class="bg-slate-950 p-4 border border-slate-800 rounded-xl">
                            <label class="font-bold text-white block mb-2">Endereço da Obra</label>
                            <input type="text" name="cliente_cep_entrega" placeholder="CEP Obra" class="w-full mb-2 px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white">
                            <textarea name="cliente_endereco_entrega" rows="2" placeholder="Endereço Obra..." class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"></textarea>
                        </div>
                    </div>
                    <div class="grid sm:grid-cols-4 gap-3 bg-slate-950 p-4 border border-slate-800 rounded-xl">
                        <textarea name="descricao_manual" rows="2" placeholder="Memorial Descritivo / Acabamentos" class="sm:col-span-4 px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"></textarea>
                        <input type="text" name="forma_pagamento" placeholder="Forma Pagamento" class="sm:col-span-2 px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white">
                        <input type="number" step="0.5" name="desconto_pct" placeholder="Desconto (%)" class="px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white">
                        <input type="number" step="10" name="entrada_valor" placeholder="Entrada (R$)" class="px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white">
                    </div>
                    <button type="submit" class="w-full py-3.5 bg-amber-500 font-bold rounded-xl text-slate-950">💾 Salvar e Liberar Contrato</button>
                </form>
            </div>
        </div>
        ...
        <div id="aba-adendo" class="tab-content space-y-6">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow">
                <h2 class="text-base font-bold text-white mb-4">➕ Termo Aditivo</h2>
                <form action="/salvar-adendo" method="post" class="grid sm:grid-cols-3 gap-4 text-xs">
                    ...
                    <input type="number" step="10" name="adendo_valor" required placeholder="Valor Extra R$" class="px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white">
                    <textarea name="adendo_descricao" rows="2" required placeholder="Descrição do aditivo..." class="sm:col-span-3 px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-white"></textarea>
                    <button type="submit" class="col-span-full py-3 bg-amber-500 font-bold rounded-xl text-slate-950">Salvar Aditivo</button>
                </form>
            </div>
        </div>
...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col justify-between font-sans">
    ...
    <main class="max-w-3xl w-full mx-auto p-4 sm:p-6 my-auto">
        <form action="/enviar-solicitacao-lead" method="post" enctype="multipart/form-data" class="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl space-y-4">
            <h2 class="text-lg font-bold text-white mb-2">Simulador de Projeto</h2>
            <div class="grid sm:grid-cols-2 gap-3 text-xs">
                ...
                <input type="number" step="any" name="area_m2_total" value="180.0" placeholder="Metragem" class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-white">
                <input type="text" name="cidade" required placeholder="Cidade" class="w-full px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-white">
            </div>
            
            <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 text-xs">
                <h3 class="font-bold text-amber-400 uppercase">Acabamentos</h3>
                <div class="grid sm:grid-cols-2 gap-3">
                    <select name="fabricante_mdf" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"><option>Duratex</option><option>Arauco</option><option>Guararapes</option></select>
                    <select name="cor_mdf" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"><option>Freijó</option><option>Carvalho</option><option>Nogueira</option><option>Branco TX</option></select>
                    <select name="marca_ferragens" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"><option>Blum (Áustria)</option><option>Hettich (Alemanha)</option><option>FGVTN</option></select>
                    <select name="modelo_portas" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"><option>Perfil Gola Alumínio</option><option>Cava Usinada</option><option>Lisa Tradicional</option></select>
                    <select name="espessura_caixa" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white"><option>MDF 18mm</option><option>MDF 15mm</option></select>
            ...
            <button type="submit" class="w-full py-4 bg-amber-500 font-bold rounded-xl text-slate-950 text-sm shadow-lg">⚡ Simular & Receber Proposta</button>
        </form>
    </main>
</body>
</html>"""
...
def render_convite_gerado(nome, email, p, tel, link):
    return f"<html><body style='background:#0f172a; color:#fff; text-align:center; padding:50px;'><h1 style='color:#f59e0b;'>Convite Gerado</h1><p>Link: {link}</p><a href='/painel' style='color:#38bdf8;'>Voltar</a></body></html>"
...
async def importar_promob(
    ...
    desc_auto = f"Projeto Promob importado. {len(calc['items'])} componentes detectados."
...
async def submit_lead(
    ...
    """, (
        agora, nome, whatsapp, nome_amb_str, (date.today() + timedelta(days=30)).strftime("%Y-%m-%d"),
    ...
    zap_url = f"https://api.whatsapp.com/send?phone=55{empresa['telefone'].replace('-','').replace(' ','').replace('(','').replace(')','')}&text=Olá"
    ...
    if orc:
        ...
        cursor.execute("""
            UPDATE orcamentos SET
                ...
                desconto_autorizado = ?, status = ?, preco_venda = ?, lucro_liquido = ?, 
        ...
        """, (
            ...
            desconto_autorizado, status, pv_final, lucro_final, forma_pagamento, 
...
def criar_usuario_com_convite(request: Request, nome: str = Form(...), email: str = Form(...), perfil: str = Form(...), telefone: str = Form("")):
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO usuarios (email, senha, nome, perfil, empresa_id, token_primeiro_acesso, primeiro_acesso_concluido) VALUES (?, '', ?, ?, 1, ?, 0)", (email, nome, perfil, token))
    conn.commit()
    conn.close()
...
def tela_primeiro_acesso(token: str):
    ...
    user = cursor.fetchone()
    conn.close()
    if not user: return HTMLResponse("Link inválido", status_code=404)
