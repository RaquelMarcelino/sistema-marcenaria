...
def render_dashboard_view():
    ...
    for h in leads:
        ...
        lucro_col = f"<td class='py-3 px-4 text-right text-emerald-400 font-bold'>R$ {lucro:,.2f}</td>" if is_admin else ""
        ...
        <div id="aba-cadastro-contrato" class="tab-content space-y-6">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow space-y-6">
                <div>
                    ...
                    <p class="text-xs text-slate-400">Preencha todos os dados pessoais, endereço com busca automática por CEP, dados bancários e memorial descritivo.</p>
                </div>
                
                <form action="/salvar-dados-completos-cliente" method="post" class="space-y-6 text-xs">
                    <!-- SELETOR DO CLIENTE / NOVO CADASTRO -->
                    <div class="bg-slate-950 p-4 rounded-2xl border border-slate-800">
                        <label class="block text-amber-400 font-bold mb-1">Selecione o Projeto / Lead existente ou crie um novo:</label>
                        <select name="orcamento_id" class="w-full px-3 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-white font-medium">
                            {options_leads}
                        </select>
                    </div>
                    ...
                    <!-- 3. DADOS BANCÁRIOS, RENDA & REFERÊNCIAS -->
                    <div class="space-y-3 pt-2">
                    ...
                    <!-- 4. MEMORIAL DESCRITIVO & CONDIÇÕES COMERCIAIS -->
                    <div class="space-y-3 pt-2">
    ...
    if orcamento_id == 0:
        ...
        cursor.execute("""
            INSERT INTO orcamentos (
                ...
                cliente_ambiente, descricao_manual, desconto_pct, desconto_autorizado, status, preco_venda,
                lucro_liquido, forma_pagamento, entrada_valor, num_parcelas, prazo_entrega, data_entrega_prevista
            ) VALUES (
                1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
